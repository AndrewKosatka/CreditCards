                ...
                cardTypeSelector.getElement('g').setAction(click -> {
                    if(click.getElement().getItem(click.getWhoClicked(), click.getSlot()) != null) {
                        ...
                        if (cardTypes.containsKey(cardType)) {
                            ...
                            sender.sendMessage(Lang.cardtype_doesnt_exists.replace("%input%", cardType).replace("%cardtypes%", String.valueOf(plugin.getCardTypesLoader().getCardTypes().keySet()).replace("[", "").replace("]", "")));
